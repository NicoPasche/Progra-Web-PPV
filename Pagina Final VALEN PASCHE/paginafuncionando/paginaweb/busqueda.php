...
<body>
   ...
   session_start();
      ...
      if ( isset( $_SESSION['id'] ) ) 
        ...
        $cancelar='can'.$row["id"];
        ?>
      <button id="<?php  echo $c ?>" onclick ="this.style.display='none' ; document.getElementById('<?php echo $d ?>').style.display='block'; document.getElementById('<?php echo $cancelar ?>').style.display='block'">Comprar</button>
      <form method="post" id="<?php  echo $d ?>" style="display: none" action="compra.php">
        <div> Esta seguro que desea comprar esta propiedad?</div>
        <button name="compra" value="<?php  echo $d ?>">Comprar</button>
        <div> o </div>
      </form>
